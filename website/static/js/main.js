function showDirectory(data) {
    ...
    // Step 2: Sort the array based on the 'upload_date' values
    let entries = Object.entries(data);
    let folders = entries.filter(([_, value]) => value.type === 'folder');
    let files = entries.filter(([_, value]) => value.type === 'file');
    ...
    // Enable single-click for opening files and folders
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    inputs.forEach(inputId => {
        document.getElementById(inputId).addEventListener('input', validateInput);
    });
